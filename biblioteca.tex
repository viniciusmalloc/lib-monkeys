...
\includefile{c++}{Multiplicative Inverse}{math}{inverse.cpp}
...
\includefile{c++}{Euler's phi function (Números co-primos (Código no livro))}{meh}{phi.cpp}
...
\includefile{c++}{Manacher (Maior substring palíndromo)}{string}{manacher.cpp}
...
\newpage
\includefile{c++}{Gabow (Emparelhamento máximo em grafos ponderados)}{graph}{gabow.cpp}
\includefile{c++}{Union Find}{graph}{union_find.cpp}

\newpage
% \includefile{c++}{Gomory Hu Tree}{graph}{gomory_hu.cpp}
\includefile{c++}{Kuhn-Munkres (Emparelhamento máximo em grafos ponderados bipartidos)}{graph}{kuhn_munkres.cpp}
\includefile{c++}{Link Cut Tree}{graph}{link_cut.cpp}
\includefile{c++}{Heavy Light Decomposition (Menor ancestral comum)}{graph}{heavy-light.cpp}
\includefile{c++}{LCA / PD (Menor ancestral comum)}{graph}{lcapd.cpp}
\includefile{c++}{Dijkstra}{graph}{dijkstra.cpp}
...
\includefile{c++}{Matrix Exponentiation}{math}{matrix_exp.cpp}
...
\includefile{c++}{Knapsack problem (Mochila)}{dp}{knapsack.cpp}
...
% \includefile{c++}{Fast Input (Leitura rápida)}{misc}{fast_input.cpp}
\newpage
